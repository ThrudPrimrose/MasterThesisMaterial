...
for i in range(4):
    offset = "0." + str(2*i)
    with open(f"col-wise-iter-{i}.tex", 'w') as file:
...
\\begin{{tikzpicture}}
    ...
    \\draw [thick,draw=BlueViolet!40!black] (.8+{offset},0) rectangle (1.0+{offset},1.2);
    \\filldraw [fill=BlueViolet!10!white,draw=BlueViolet!40!black] (.8+{offset},0) rectangle (1.0+{offset},1.2);
    \\draw [step=0.4/2, very thin, color=gray] (.8+{offset},0) grid(1.0+{offset},1.2);
    ...
    % Code Col-wise, iter 1
    \\draw [thick,draw=BlueViolet!40!black] (2,2.2-{offset}) rectangle (2.2,2.4-{offset});
    \\filldraw [fill=BlueViolet!10!white,draw=BlueViolet!40!black] (2,2.2-{offset}) rectangle (2.2,2.4-{offset});
    \\draw [step=0.4/2, very thin, color=gray] (2, 2.2-{offset}) grid (2.2,2.4-{offset});
    ...
    % Code Col-wise, iter 1
    \\draw [thick,draw=BlueViolet!{40+float(offset)*40}!black] (2,0) rectangle (2.2,1.2);
    \\filldraw [fill=BlueViolet!{10+float(offset)*40}!white,draw=BlueViolet!{40+float(offset)*40}!black] (2,0) rectangle (2.2,1.2);
...
for i in range(4):
    ...
    command = f"pdflatex col-wise-iter-{i}.tex"
    result = subprocess.run(command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    command = f"inkscape -l col-wise-iter-{i}.pdf --export-filename=col-wise-iter-{i}_svg.svg"    
    result = subprocess.run(command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
