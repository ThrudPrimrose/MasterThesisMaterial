...
int main(){{
    ...
    cudaMemcpy((void *)C2_dev, (void *)C, sizeof(float) * {rowCT} * {colCT} * {num_els}, cudaMemcpyHostToDevice); CHECK_ERR;

    float** C2_begins_cusparse = new float*[{num_els}];
    float** C2_dev_begins_cusparse = nullptr;
    cudaMalloc((void **)&C2_dev_begins_cusparse, {num_els} * sizeof(float*));
    for (int i = 0; i < {num_els}; i++){{
        C2_begins_cusparse[i] = &C2_dev[0] + {rowCT} * {colCT} * i;
    }}
    float** A_begins_cus = new float*[{num_els}];
    float** B_dense_begins_cus = new float*[{num_els}];
    float** C2_begins_cus = new float*[{num_els}];
    float** C3_begins_cus = new float*[{num_els}];
    float** A_dev_begins_cus = nullptr;
    float** B_dense_dev_begins_cus = nullptr;
    float** C2_dev_begins_cus = nullptr;
    float** C3_dev_beings_cus = nullptr;
    for (int i = 0; i < {num_els}; i++){{
        A_begins_cus[i]  = &A_dev_cus[0]  + {rowA} * {colA} * i;
        B_dense_begins_cus[i]  = &B_dense_dev[0]  + {rowB} * {colB} * i;
        C2_begins_cus[i] = &C2_dev[0] + {rowC} * {colC} * i;
    }}
    cudaMalloc((void **)&A_dev_begins, {num_els} * sizeof(float*)); CHECK_ERR;
    cudaMalloc((void **)&B_dense_dev_begins, {num_els} * sizeof(float*)); CHECK_ERR;
    cudaMalloc((void **)&C2_dev_begins, {num_els} * sizeof(float*)); CHECK_ERR;
    ...
    cudaMemcpy((void *)C2_dev_begins_cusparse, (void *)C2_begins_cusparse, {num_els} * sizeof(float*), cudaMemcpyHostToDevice); CHECK_ERR;
    delete[] C2_begins_cusparse;
    ...
    CHECK_CUSPARSE( cusparseCreateDnMat(&matCT, {colCT}, {rowCT}, {colCT}, C2_dev,
                                        CUDA_R_32F, CUSPARSE_ORDER_COL) )
    ...
    cuBlasStatus = cublasSgemmBatched(handle, CUBLAS_OP_T, CUBLAS_OP_N,
        {rowA}, {colB}, {colA}, (const float*)&transpose_alpha, (const float **)C2_dev_begins, {rowA}, (const float **)B_dense_dev_begins, {rowB},
        (const float*)&transpose_beta, (float **)C2_dev_begins, {rowC}, {num_els}); CHECK_ERR;
    if (cuBlasStatus != CUBLAS_STATUS_SUCCESS) {{
        std::cout << "Second cuBlas call failed";
    }}
    ...
    cudaFree(C2_dev_begins_cusparse);
    delete[] AT;
    delete[] CT;
    delete[] BT_data;
    delete[] BT_indices;
