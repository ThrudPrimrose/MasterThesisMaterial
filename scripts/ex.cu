...
int main(void) {
    ...
    for (int i = 0; i < A_num_rows; i++) {
        for (int j = 0; j < B_num_cols; j++) {
            if (hC1[i + j * ldc] > 1.01*hC1_result[i + j * ldc] or hC1[i + j * ldc] < 0.99*hC1_result[i + j * ldc]) {
                correct = 0; // direct floating point comparison is not reliable
                printf("%f != %f\n", hC1[i + j * ldc], hC1_result[i + j * ldc]);
                break;
            }
            if (hC2[i + j * ldc] > 1.01*hC2_result[i + j * ldc] or hC2[i + j * ldc] < 0.99*hC2_result[i + j * ldc]) {
                correct = 0;
                printf("%f != %f\n", hC2[i + j * ldc], hC2_result[i + j * ldc]);
                break;
            }
        }
    }
